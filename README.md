# 🏥 병원 로봇 원격 제어 통합 시스템 (Hospital Robot System)

Flask + ROS2 + MySQL + YOLOv8 기반 병원 로봇 원격 제어 시스템입니다.
병동을 자율 순찰하는 ROS2 로봇(TurtleBot4)과, 관리자가 환자/병실 정보를 관리하고 로봇 상태를 실시간으로 확인하는 Flask 웹 대시보드로 구성되어 있습니다.
...
이 프로젝트는 팀 프로젝트이며, 저는 **웹 대시보드(Flask 서버)** 부분을 담당했습니다.

- **로그인 / 인증** — MySQL `users` 테이블 연동, 세션 관리, DB 연결 실패 시 폴백 로그인
- **대시보드** (`/main`, `/api/dashboard/summary`) — 병동·환자 현황 집계, 상태 배지(safe/warning/danger) 로직
- **환자·병실 CRUD** — `/api/patient`, `/api/room`, `/api/emergency`
- **Mock DB 설계** (`_MockDB` 클래스) — MySQL 미연결 시에도 서버가 정상 동작하도록 인메모리 DB 자동 전환 구현

robot6 제어, 코드 블루(Nav2), YOLO 카메라 스트리밍, 그리고 아래 ROS2 순찰 패키지(TurtleBot4 자율 순찰, YOLO Pose 사람 감지, 음성 안내)는 다른 팀원이 담당했습니다.
...
1. [프로젝트 구조](#-프로젝트-구조)
2. [주요 기능 (웹 대시보드)](#-주요-기능-웹-대시보드)
3. [실행 환경](#-실행-환경)
4. [설치 및 실행 (웹 대시보드)](#-설치-및-실행-웹-대시보드)
5. [MySQL 설정](#-mysql-설정)
6. [LOCATION_DB 설정](#-location_db-설정)
7. [웹 페이지 및 API](#-웹-페이지-및-api)
8. [데이터베이스 스키마](#-데이터베이스-스키마)
9. [ROS2 토픽](#-ros2-토픽)
10. [ROS2 순찰 패키지 (rokey_patrol)](#-ros2-순찰-패키지-rokey_patrol)
11. [실행 모드](#-실행-모드)
12. [트러블슈팅](#-트러블슈팅)
13. [변경 이력](#-변경-이력)
...
├── hospital_robot_server/      # 웹 대시보드 (Flask + ROS2 통합)
│   ├── app.py                  # 메인 서버
...
│
└── rokey_patrol/                # ROS2 순찰 패키지 (TurtleBot4)
    ├── amr_detect.py            # YOLO Pose 기반 사람/상황 감지
    ├── enji_auto_localization.py # AprilTag 기반 위치 보정
    └── patrol_node_april.py     # 순찰 메인 노드
```
...
| TurtleBot4 Navigation / Nav2 | Humble 기준 | 🔶 ROS2 순찰 패키지 실행 시 필요 |
...
## 🚶 ROS2 순찰 패키지 (rokey_patrol)

> TurtleBot4 기반 병원 자율 순찰 로봇 시스템입니다. Nav2 기반 자율 주행, YOLO Pose 기반 사람 감지, 상황별 음성 안내 기능을 포함하며, 위 웹 대시보드와 ROS2 토픽으로 연동됩니다. 이 부분은 팀원이 담당했습니다.

### 기술 스택
- ROS2, TurtleBot4 Navigation, Nav2
- YOLOv8 Pose

### 주요 기능
- TurtleBot4 기반 자율 순찰 및 내비게이션
- YOLO Pose 기반 사람/상황 감지
- 위치 자동 추정
- 상황별 음성 안내
...
웹 개발과 인증/보안 구현이 처음이라 전반적으로 막히는 부분이 많았지만, 하나씩 찾아가며 해결했습니다.

**비밀번호 보안 처리** — 초기에는 사용자 비밀번호를 평문으로 저장하고 있었는데, 보안상 문제가 있다는 걸 인지하고 해싱 처리를 적용해 비밀번호 원문이 DB에 노출되지 않도록 개선했습니다.

**환자/병실 CRUD** — 데이터를 등록, 수정, 삭제하는 기본적인 기능을 구현하면서 입력값 처리와 DB 반영 로직을 반복적으로 테스트하고 다듬었습니다.

**Mock DB 전환 로직** — 실제 로봇이나 DB 서버가 연결되지 않은 환경에서도 대시보드 동작을 테스트할 수 있도록 `_MockDB` 클래스를 만들어, 실제 DB 연결이 실패하거나 없을 때 자동으로 대체 데이터를 제공하도록 처리했습니다.

**대시보드 통계 집계** — 환자 상태, 병실 현황 등 여러 테이블의 데이터를 종합해서 보여주는 통계 API를 만드는 과정에서, 데이터를 어떻게 집계하고 어떤 형태로 프론트엔드에 전달할지 여러 차례 구조를 바꿔가며 정리했습니다.

추가로 아래는 자주 발생했던 환경/연동 문제와 해결 방법입니다.
